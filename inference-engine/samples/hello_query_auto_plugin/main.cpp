...
//            if (metricName != METRIC_KEY(AVAILABLE_DEVICES)) {
              if (true) {
                ...
                printParameterValue(ie.GetMetric(deviceName, metricName));
            }
        }
        ...
        if (std::find(supportedMetrics.begin(), supportedMetrics.end(),
            ...
            std::vector<std::string> supportedConfigKeys = ie.GetMetric(deviceName, METRIC_KEY(SUPPORTED_CONFIG_KEYS));
            for (auto && configKey : supportedConfigKeys) {
                std::cout << "\t\t" << configKey << " : " << std::flush;
                printParameterValue(ie.GetConfig(deviceName, configKey));
            }
        }
